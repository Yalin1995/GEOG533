...
nrow <- nrow(df)
id <- 1:nrow
train.index <- sort(sample(id,size = nrow/2,replace = FALSE))
train.index
...
**See texbook (Statistical Methods for Geography) page 114**
...
```{r}
pexp(3, rate = 1/4.3, lower.tail = FALSE)
```
...
```{r}
pexp(7, rate = 1/4.3,lower.tail = FALSE) / pexp(4, rate = 1/4.3, lower.tail = FALSE)
```
